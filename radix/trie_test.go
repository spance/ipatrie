...
func verify(t testing.TB, trie *Trie, addr string, expected int) {
	found := trie.Lookup(ParseIP(addr))

	if expected >= 0 && found != uint32(expected) {
		t.Errorf("Expected [%s]==%v but got=%v", addr, expected, found)
	}
	if expected < 0 && found != NULL {
...
func TestSimple(t *testing.T) {
	...
	trie := NewTrie()
	for i, s := range net_samples {
		a, m := ParseCIDR(s)
		trie.Add(a, byte(m), uint32(i+1))
	}
	trie.printStat(t)
...
func initTestData(file string) *Trie {
	...
	fileIter(file, func(fields []string) {
		na, m := ParseCIDR(fields[0])
		nu := parseField1(fields[1])
		trie.Add(na, byte(m), uint32(nu))
	})
	return trie
}
